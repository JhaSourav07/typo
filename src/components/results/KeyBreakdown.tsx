...
export const KeyBreakdown: React.FC<KeyBreakdownProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full my-4">
      <div className="bg-[var(--bg-sub)] rounded-xl border border-[var(--border)] p-4">
        <span className="text-xs font-semibold text-[var(--text-main)] block mb-3">
          Keystroke Statistics
        </span>
        <div className="grid grid-cols-2 gap-2 text-xs font-mono">
          <div className="p-2.5 rounded-lg bg-[var(--bg-card)] border border-[var(--border)] flex justify-between">
      ...
      <div className="bg-[var(--bg-sub)] rounded-xl border border-[var(--border)] p-4">
        ...
        {problemKeys.length > 0 ? (
          <div className="flex flex-wrap gap-2">
            {problemKeys.map(([char, stat]) => {
              ...
              return (
                <div
                  key={char}
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[var(--bg-card)] border border-red-500/30 text-xs font-mono"
                >
          ...
          <div className="text-xs text-[var(--text-sub)] italic py-3">
